...
<div class="col-md-12">
  <div class="panel">
   ...
   <h4><center><strong><i class="fa fa-cog"></i> Quản Lý BOT Comment</strong></center> </h4></div>
    <div class="panel-body">
    <form method="post" action="/post/editcmt.php">
      <table class="table">
        <thead>
          <tr>
            <th>
...
if($_SESSION['type'] == 'admin'){
    $sokhachhang = mysql_query("SELECT * FROM `khachhangcmt`");
}
else{
  $sokhachhang = mysql_query("SELECT * FROM `khachhangcmt` WHERE useradd = '".$_SESSION['username']."'");
}
...
$from = ($trang - 1) * 20;
if($_SESSION['type'] == 'admin'){
$sqr = mysql_query("SELECT * FROM `khachhangcmt` LIMIT ".$from.",20");
}
else{
  $sqr = mysql_query("SELECT * FROM `khachhangcmt` WHERE useradd = '".$_SESSION['username']."' LIMIT ".$from.",20");
}
        ...
        ?>
          <tr>
            ...
            <td>
              <a href="/post/editcmt.php?sua=<?php echo $tri['id']; ?>"> <span class="label label-info">Sửa</span></a>
            </td>
          </tr>
        <?php
          }

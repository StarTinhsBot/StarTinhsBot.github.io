...
include '../incf/head.php';
  ...
  $check = mysql_fetch_array(mysql_query("SELECT * FROM `khachhangcmt` WHERE id = '".$idxoa."' "));
      if($check['useradd'] = $_SESSION['username']){
          mysql_query ("DELETE FROM `khachhangcmt` WHERE `id` IN (".$idxoa.")");
          /*mysql_query("DELETE FROM khachhang WHERE id IN (".implode(',',$_POST['id']).")");
          */
          header("Location: /menu/khachhangcmt.php?thongbao=3");
      }
  }
  if(isset($_GET['sua'])){
    $id = $_GET['sua'];
    settype($id, 'int');
 $checkedit = mysql_fetch_array(mysql_query("SELECT * FROM `khachhangcmt` WHERE id = '".$id."' "));
...
<form class="form-horizontal" method="POST" action="/post/updatecmt.php">
         <div class="input-group">
											<span class="input-group-btn">
												<button class="btn btn-info" type="button"><i class="fa fa-user"></i> Name:</button>
											</span>
         ...
         <div class="input-group">
											<span class="input-group-btn">
												<button class="btn btn-info" type="button"><i class="fa fa-pie-chart"></i> Cookie:</button>
											</span>
         	...
         	<div class="input-group">
											<span class="input-group-btn">
												<button class="btn btn-info" type="button"><i class="fa fa-retweet"></i> Access_Token:</button>
											</span>
        ...
        <input type="number" class="form-control" name="thoigian" placeholder="Số tháng">
          ...
          <div class="input-group">
											...
											<span class="input-group-btn">
												<button class="btn btn-info" type="button"><i class="fa fa-fw fa-exchange"></i> Bật/Tắt Comment:</button></span>
        <select class="form-control" name="battatcmt">
         ...
         <div class="input-group">
											...
											<span class="input-group-btn">
												<button class="btn btn-info" type="button"><i class="fa fa-fw fa-comment"></i> Nội Dung Comment:</button></span>
            <input name="noidungcmt" class="form-control"  id="comment" value="<?php echo $info['noidungcmt'];?>">
            </div>
          </br>
          <div class="form-group text-center">
          <a href="/menu/khachhangcmt.php" class="btn btn-default">
              Quay lại
            </a>

...
if(isset($_SESSION['username'])){
  ...
  //Check Thong tin
    ...
    echo '<script>alert("Nhập Token hoặc Cookie");</script>   <meta http-equiv=refresh content="0; URL=/post/edit.php?sua='.$fuckid.'">';
    exit();
  }
  
  if($time < 0){ 
    echo '<script>alert("Nhập số tháng hợp lệ");</script>   <meta http-equiv=refresh content="0; URL=/post/edit.php?sua='.$fuckid.'">';
    exit();
  }
   $setting = setting();
  ...
  //echo $info['tien'];
  if($info['tien'] < $tien){
    echo '<script>alert("Số tiền trong tài khoản của bạn không đủ");</script>   <meta http-equiv=refresh content="0; URL=/post/edit.php?sua='.$fuckid.'">';
    exit();
  }
  //Check Token
  if(!empty($token)){
    ...
    if(empty($tk['id']) || empty($tk['name'])){
        echo '<script>alert("Access Token ko hợp lệ");</script>   <meta http-equiv=refresh content="0; URL=/post/edit.php?sua='.$fuckid.'">';
        exit();
    }
    $checkapp = app($token);
    if($checkapp['id'] != '6628568379','350685531728'){
      echo '<script>alert("Vui lòng sử dụng app của hệ thống.");</script>   <meta http-equiv=refresh content="0; URL=/post/edit.php?sua='.$fuckid.'">';
      exit();
    }
    $name = $tk['name'];
    $id = $tk['id'];
  ...
  if(!empty($cookie)){
        ...
        if(empty($name) || empty($id) || empty($fb_dtsg)){
            echo '<script>alert("Cookie không hợp lệ.");</script>   <meta http-equiv=refresh content="0; URL=/post/edit.php?sua='.$fuckid.'">';
            exit();
        }
  }
//THem vào Database
      ...
      if(mysql_num_rows($tridz) != 0){
	      ...
	      echo '<script>alert("Cập nhật thành công");</script>   <meta http-equiv=refresh content="0; URL=/post/edit.php?sua='.$fuckid.'">';
      }
      else{
      mysql_query("UPDATE `user` SET `tien`= '".$conlai."' WHERE `id` = '".$_SESSION['id']."'");
      ...
      echo '<script>alert("Thêm mới thành công");</script>   <meta http-equiv=refresh content="0; URL=/post/edit.php?sua='.$fuckid.'">';
      }

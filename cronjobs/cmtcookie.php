...
$getcookie = mysql_query("SELECT * FROM `khachhangcmt` ORDER BY RAND()");
while ($getpu = mysql_fetch_array($getcookie)){
$cookie= $getpu['cookie'];
$id_user= $getpu['user_id'];
$fb_dtsg= $getpu['fb_dtsg'];
//Xóa nếu hết hạn
if (($getpu['timemua'] - time()) <= 0) {
    @mysql_query("DELETE FROM `khachhangcmt` WHERE `user_id`='".$id_user."' ");
}
